...
public class PotionsTest {
    ...
    @Test
    @Tag("6-6")
    @DisplayName("Test invincibility potions cause zombies to run away")
    public void invincibilityZombieMovement() throws InvalidActionException {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
            "d_potionsTest_invincibilityZombieMovement", "c_potionsTest_invincibilityZombieMovement");
        ...
        // check that distance between zombie and player grows over time
        // the player will stay in the same position (hitting wall) but the zombie should move away
        ...
        zombiePos = TestUtils.getEntities(res, "zombie_toast").get(0).getPosition();
        int endingMagnitude = (int) Math.floor(TestUtils.getEuclideanDistance(playerPos, zombiePos));
        assertTrue(endingMagnitude >= startingMagnitude);
    }

    @Test
    @Tag("6-7")
    @DisplayName("Test invincibility potions cause mercenaries to run away")
    public void invincibilityMercenaryMovement() throws InvalidActionException {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
            "d_potionsTest_invincibilityMercenaryMovement", "c_potionsTest_invincibilityMercenaryMovement");
        ...
        // check that distance between mercenary and player grows over time
        // the player will stay in the same position (hitting wall) but the mercenary should move away
        Position playerPos = TestUtils.getEntities(res, "player").get(0).getPosition();
        Position mercenaryPos = TestUtils.getEntities(res, "mercenary").get(0).getPosition();
        int startingMagnitude = (int) Math.floor(TestUtils.getEuclideanDistance(playerPos, mercenaryPos));
        for (int i = 0; i <= 5; i++) dmc.tick(Direction.RIGHT);
